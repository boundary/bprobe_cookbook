  ...
  module API
    ...
    def apply_cloud_tags_ec2(new_resource)
      if node[:ec2]
        ...
        apply_an_tag(new_resource, node[:ec2][:placement_availability_zone]) if node[:ec2][:placement_availability_zone]

        apply_an_tag(new_resource, node[:ec2][:instance_type]) if node[:ec2][:instance_type]
      end
    end
    ...
    def apply_cloud_tags_opsworks(new_resource)
      if node[:opsworks]
        ...
        if node[:opsworks][:stack]
          apply_an_tag(new_resource, node[:opsworks][:stack][:name]) if node[:opsworks][:stack][:name]
        end
